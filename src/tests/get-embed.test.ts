...
describe('Get embed', () => {
  ...
  it.each(WebhookDefinitions.map(
    ...
    for (const [exampleId, example] of definition.examples.entries()) {
      const mockInstance = jest.spyOn(BaseAction.prototype as unknown as {
        sendMessage: (key: string, message: DiscordMessage) => Promise<void>
      }, 'sendMessage').mockImplementationOnce((_: string, message: DiscordMessage) => {
      ...
      if(response.body.includes('An error occurred: Method not implemented.')) {
        return
      }
